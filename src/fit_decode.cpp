...
FIT_BOOL Decode::Read(std::istream &file)
{
    ...
    while ( (filePosition < fileSize) && (status == FIT_TRUE) )
    {
        InitRead(file, FIT_FALSE);
        status = Resume();
        filePosition = (FIT_UINT32)file.tellg();
    }
                ...
                if ((localMesgDefs[localMesgIndex].GetFieldByIndex(fieldIndex)->GetType() & FIT_BASE_TYPE_NUM_MASK) < FIT_BASE_TYPES) // Ignore field if base type not supported.
                {
                    ...
                    if (field.IsValid()) // If known field type.
                    {
                        ...
                        if ( field.GetIsAccumulated() )
                        {
                            ...
                            {
                                FIT_FLOAT32 value = (FIT_FLOAT32)field.GetRawValue((FIT_UINT8)i);
